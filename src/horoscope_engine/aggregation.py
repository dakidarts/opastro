...
def _sample_schedule(period: Period, start: datetime, end: datetime) -> List[datetime]:
    ...
    if period == Period.MONTHLY:
        ...
        return [
            start + timedelta(days=i, hours=12)
...
def _eclipse_distances(snapshot: ChartSnapshot) -> Optional[Tuple[float, float, float]]:
    ...
    sun_node_dist = min(
        _angle_distance(sun_lon, north_lon), _angle_distance(sun_lon, south_lon)
    )
    moon_node_dist = min(
        _angle_distance(moon_lon, north_lon), _angle_distance(moon_lon, south_lon)
    )
    return sun_moon_sep, sun_node_dist, moon_node_dist


def _section_bias(
    event_type: str, body1: Optional[str], body2: Optional[str]
) -> Dict[str, float]:
...
def _event_priority(
    ...
    body_weight = max(
        BODY_IMPORTANCE.get(body1 or "", 1.0), BODY_IMPORTANCE.get(body2 or "", 1.0)
    )
    exact_bonus = 0.0
...
def _event_signature(
    event: PeriodEvent,
) -> Tuple[str, Tuple[str, ...], Optional[str], Optional[str], Optional[str]]:
...
def _dedupe_period_events(
    period: Period, period_events: List[PeriodEvent]
) -> List[PeriodEvent]:
    # First pass: remove strict duplicates by event_type/body/sign/aspect/date.
    strict: List[PeriodEvent] = []
    seen_keys = set()
    for event in sorted(
        period_events, key=lambda e: (e.timestamp, -e.narrative_priority)
    ):
    ...
    for event in strict:
        ...
        for idx, existing in enumerate(bucket):
            delta_days = (
                abs((event.timestamp - existing.timestamp).total_seconds()) / 86400.0
            )
...
def aggregate_period(
    ...
    for sample in samples:
        snapshot = ephemeris.chart_snapshot(
            sample, include_houses=include_houses, coordinates=coordinates
        )
        ...
        for aspect in snapshot.aspects:
            ...
            if aspect.exact and aspect.aspect in MAJOR_EXACT_ASPECTS:
                ...
                if _is_nodal_pair(aspect.body1, aspect.body2):
                    continue
                description = f"{aspect.body1} {aspect.aspect} {aspect.body2} exact on {sample.strftime('%b %d')}"
                period_events.append(
                    PeriodEvent(
                        ...
                        narrative_priority=_event_priority(
                            "exact_aspect", aspect.body1, aspect.body2, aspect.orb
                        ),
                        section_bias=_section_bias(
                            "exact_aspect", aspect.body1, aspect.body2
                        ),
        ...
        for position in snapshot.positions:
            if position.retrograde:
                retrograde_bodies[position.name] = (
                    retrograde_bodies.get(position.name, 0) + 1
                )

        if previous_snapshot:
            for position in snapshot.positions:
                prev = next(
                    (p for p in previous_snapshot.positions if p.name == position.name),
                    None,
                )
                if not prev:
                    continue
                if prev.sign != position.sign:
                    ...
                    period_events.append(
                        PeriodEvent(
                            ...
                            sign=position.sign,
                            narrative_priority=_event_priority(
                                "ingress", position.name, None, None
                            ),
                ...
                if not prev.retrograde and position.retrograde:
                    ...
                    period_events.append(
                        PeriodEvent(
                            ...
                            body1=position.name,
                            narrative_priority=_event_priority(
                                "station", position.name, None, None
                            ),
                ...
                if prev.retrograde and not position.retrograde:
                    if position.name in NODAL_BODIES:
                        continue
                    description = (
                        f"{position.name} stations direct on {sample.strftime('%b %d')}"
                    )
                    period_events.append(
                        PeriodEvent(
                            timestamp=sample,
        ...
        if eclipse_dist is not None:
            ...
            is_solar = (
                min(sun_moon_sep, abs(360.0 - sun_moon_sep)) <= 12.0
                and sun_node_dist <= 18.0
            )
            is_lunar = abs(sun_moon_sep - 180.0) <= 12.0 and moon_node_dist <= 18.0
            if is_solar or is_lunar:
                description = f"{'Solar' if is_solar else 'Lunar'} eclipse window on {sample.strftime('%b %d')}"
                ...
                period_events.append(
                    PeriodEvent(
                        ...
                        narrative_priority=_event_priority(
                            "eclipse",
                            "Sun" if is_solar else "Moon",
                            "North Node",
                            exactness,
                        ),
                        section_bias=_section_bias(
                            "eclipse", "Sun" if is_solar else "Moon", "North Node"
                        ),
                        description=description,
                    )
                )
    ...
    snapshot = (
        snapshots[len(snapshots) // 2] if snapshots else ephemeris.chart_snapshot(start)
    )
    ...
    for name in retrograde_bodies.keys():
        ...
        unique_events.append(
            PeriodEvent(
                ...
                body1=name,
                narrative_priority=_event_priority(
                    "retrograde_emphasis", name, None, None
                ),
